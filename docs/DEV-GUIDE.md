...
|-------|-----------|-------------|--------|
| KFC-R01 | login-compliance | `test_login_compliance.sh` and `login-compliance-check.sh` existed in both `check-for-updates/` and `login-compliance/`. Duplicates removed; canonical copies now in `login-compliance/` only. **Regressed (found 2026-06-03):** the two source copies had diverged — the canonical `login-compliance/` copy held the obsolete `apt-get dist-upgrade -s` + XDG-cache implementation, while the correct state-file-reading implementation sat in the `check-for-updates/` duplicate that was supposed to have been deleted, and the installer deployed the obsolete one. Re-closed in login-compliance v0.10.0: the state-file-reading implementation is now the sole canonical copy under `login-compliance/src/`; the `check-for-updates/src/` and `check-for-updates/tests/unit/` duplicates were deleted. | Closed (re-closed v0.10.0) |
